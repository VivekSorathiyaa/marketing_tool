...
class NetworkDioHttp {
  ...
  static Future<Map<String, String>> getHeaders() async {
    final String? token = await AppPreferences.getAuthToken();
    ...
    if (internet) {
      ...
      try {
        ...
        if (e.response != null && e.response!.statusCode == 498) {
          await AppPreferences.clear().whenComplete(() {
            getx.Get.offAll(() => DashBoardScreen());
          });
        }
    ...
    if (internet) {
      try {
        ...
        Response response = await _dio!
            .post("$url", data: formData, options: _cacheOptions);
        if (response.statusCode == 200) {
          if (context != null) processIndicator.hide(context);
          return {
            'body': json.decode(json.encode(response.data)),
            'headers': response.headers,
            'error_description': null,
          };
        } else {
